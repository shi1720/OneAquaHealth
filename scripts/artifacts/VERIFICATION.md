...
- Both PDFs include the verified hosted application URL, `https://rill-streams.web.app`. Architecture text describes Firebase Hosting, Cloud Run and PostgreSQL on Cloud SQL. Notes distinguish local software checks from field validation and point to the versioned QA record for current CI evidence. The final release CI result was added to that record after artifact finalization; the deck and PDFs remain unchanged.
