...
// Structural D1 interface avoids bringing Worker globals into the Node build.
...
export class D1Storage implements Storage {
  ...
  private stmt(sql: string, params: SQLValue[] = []) {
    return this.db.prepare(sql).bind(...params);
  }
  async all<T>(sql: string, params?: SQLValue[]) {
    return ((await this.stmt(sql, params).all()).results ?? []) as T[];
  }
  async get<T>(sql: string, params?: SQLValue[]) {
    return ((await this.stmt(sql, params).first()) ?? undefined) as T | undefined;
  }
  async run(sql: string, params?: SQLValue[]) {
    return { changes: (await this.stmt(sql, params).run()).meta?.changes ?? 0 };
  }
  async batch(statements: SQLStatement[]) {
    return (await this.db.batch(statements.map((item) => this.stmt(item.sql, item.params)))).map(
      (result) => ({ changes: result.meta?.changes ?? 0 }),
    );
  }
}
